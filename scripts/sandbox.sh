...
echo -ne '\033[1;34m SANDBOX ENVIRONEMENT\n'

echo 'Changing path to /production'
cd /var/www/production

echo 'Checking out on dev'
...
sudo npm run build -- --env.sandbox

echo -ne '\033[1;32m Copying source -> dist/\n'
sudo cp -a /var/www/production/dist/source/. /var/www/production/dist/build/

echo -ne '\033[1;33m Clearing sandbox folder\n'
sudo rm -rf /var/www/production/dist/sandbox

echo -ne '\033[1;33m Moving tmp to sandbox folder\n'
sudo mv /var/www/production/dist/build /var/www/production/dist/sandbox
